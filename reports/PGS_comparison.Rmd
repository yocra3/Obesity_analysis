...
```{r}
version <- "2021-06-18"
datasets <- c("Locke", "PGS000027", "EGG", "SCOOP_obesity", "SCOOP_extreme")
...
pgs_tible <- Reduce(rbind, pgs_list) %>%
  ...
  mutate(PGS_dataset = factor(PGS_dataset, levels = c("SCOOP_extreme", "SCOOP_obesity", "EGG", "PGS000027", "Locke")))
```
```{r}
pgs_tible %>%
...
pgs_assoc_raw <- Reduce(rbind, pgs_assoc_raw) %>%
  as_tibble() %>%
  mutate(PGS_dataset = factor(datasets, levels = c("SCOOP_extreme", "SCOOP_obesity", "EGG", "PGS000027", "Locke")),
         model = "Raw")
